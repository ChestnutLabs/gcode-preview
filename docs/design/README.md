...
| [DD-012](DD-012-non-extrusion-toolpath-and-modal-tool-state.md) | Non-extrusion toolpath (CNC/laser/plotter) & modal tool-state channels | #189 | Future | **Accepted** 2026-07-28 (D1–D8 as recommended) — additive `MoveKind.Cut` + one opt-in `ModalChannel` mechanism (owns tool-state **and** #180/RR-002 color channels → DD-015 retired), canned-cycle (`G81–89`) expansion, validation-tiered dialects; per [RR-004](../research/RR-004-non-extrusion-toolpath-coverage.md) |
...
| [DD-020](DD-020-interaction-aware-quality.md) | Interaction-aware render quality (progressive LOD) | #306 | M1 | **Accepted** 2026-08-24 — opt-in `interactionQuality:'auto'`: reduce render detail (pixel ratio, bounded frame-time step) while the camera moves, restore on settle; keeps the vertex-budget fallback; consumer maps High/Balanced/Performance. Additive, default off. |
| [DD-018](DD-018-model-renderer.md) | ModelRenderer — source-model presentation rendering (STL/3MF) | ModelRenderer (new epic) | M1 | **Accepted** 2026-08-23 (all sections as proposed; §14 v1 = both STL+3MF sequential; §12 shared stage stays in gcode-renderer-three short-term) — new sibling `@chestnutlabs/gcode-model-renderer` for presentation/thumbnail renders of source STL/3MF (distinct from toolpath inspection); shared "stage" extraction, headless/offscreen (SwiftShader) contract with transparent+themed bg, multi-object + material-capable scene model from day one, deterministic export + `(sourceHash, options, envId)` cache key; consumer: AnyBridge thumbnail sidecar |
| [DD-021](DD-021-interactive-model-viewer.md) | Interactive source-model viewer (`ModelViewer`) | ModelRenderer (DD-018 line) | M2 | **Accepted** 2026-08-24 (maintainer; AnyBridge boundary-reviewed, no blockers) — a distinct interactive `createModelViewer(canvas)` handle (orbit/zoom/pan of source STL/3MF, the live analogue of `renderModelStill`); reuses the shared stage + `ModelRenderer` scene core + the toolpath camera/controls + DD-020 quality; **generic open-`kind` model-source loader registry** (STL/3MF first, OBJ/STEP/PLY later w/o API change); distinct from ToolpathRenderer / `<GcodePreview>`; handle in `gcode-model-renderer`, shared-control extraction into `gcode-renderer-three`; components a later phase; consumer: AnyBridge "View in 3D" |
| [DD-022](DD-022-model-instancing-and-lod.md) | Instance-aware source-model rendering (GPU instancing + LOD) | ModelRenderer (DD-018 line) | M3 | **Accepted** 2026-08-25 (both-owner gate satisfied — maintainer + AnyBridge product owner on §4 + §8, incl. the headless/SwiftShader fix). Full-plate source `.3mf` reuses master meshes via production-extension components / repeated build items, but `parse3mf` **bakes every instance** → ~26 M triangles → `E_MODEL_TOO_MANY_TRIANGLES` after ~10 s (proof: `Baby_Opossum` full sheet). Per the AnyBridge owner's 6 constraints (NOT "raise the limit"): instance-aware `ModelScene` (additive `instances?` on `ModelObject`) drawn via `InstancedMesh`, byte-estimate **fast-reject**, LOD/decimation for huge **unique** meshes (disclosed, an RR-006 analogue), a **hard ceiling** as final guardrail (now on unique triangles), and **client-capability-aware** interactive degradation (software-WebGL degrades harder; headless assumes server GPU). Renderer-owned in `gcode-model-renderer`; consumer: AnyBridge "View in 3D" |
