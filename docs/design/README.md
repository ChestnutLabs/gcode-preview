...
| [DD-018](DD-018-model-renderer.md) | ModelRenderer — source-model presentation rendering (STL/3MF) | ModelRenderer (new epic) | M1 | **Proposed** 2026-08-23 — new sibling `@chestnutlabs/gcode-model-renderer` for presentation/thumbnail renders of source STL/3MF (distinct from toolpath inspection); shared "stage" extraction, headless/offscreen (SwiftShader) contract with transparent+themed bg, multi-object + material-capable scene model from day one, deterministic export + `(sourceHash, options, envId)` cache key; v1 = STL proving slice → 3MF multi-object/color; consumer: AnyBridge thumbnail sidecar |
