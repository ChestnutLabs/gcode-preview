...
| 2026-07-25 | **Phase 2 (#157) shipped** — arc-plane selection (G17/G18/G19), plane-parameterized `g2`, deferred G91-arc geometry, `arcPlanes` capability; XY-arc corpus byte-identical | Chestnut Labs |
| 2026-07-25 | **D4 amended (maintainer) — probe-aware datum.** Honoring `G92` X/Y/Z as a bare datum shift mis-rendered a real plasma fixture whose `G92 Z0` follows an un-modeled `G31` probe (shifted Z by the stale pre-probe height). D4 now tracks **per-axis position certainty**: `G31` marks the probed axis runtime-dependent (`probe-position-runtime-dependent`, no fabricated endpoint); a following `G92` is a logical **resync** (new frame at the datum, no move drawn across the probe), while a `G92` on a known position keeps the continuity datum-shift. Phase 3 (#158) shipped on this basis — G53/G54–G59/G10 + G92 XYZ + probe awareness + `coordinateSystem` capability; identity-WCS corpus byte-identical, `mach3` an intentional documented divergence. **E10 audit gaps all closed.** | Maintainer + Chestnut Labs |