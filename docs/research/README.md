...
| [RR-005](RR-005-3mf-paint-color-facet-format.md) | 3MF `paint_color` facet-paint format (Bambu/Orca production multicolor) | DD-018 follow-up | **Complete.** Reverse-engineered the proprietary per-triangle `paint_color` encoding **clean-room** from a real file (no Orca/PrusaSlicer GPL): a little-endian nibble-token recursive split-tree; leaf `state 0` = default extruder, `state s ≥ 1` = filament index `s−1`; palette in `project_settings.config filament_colour`. Validated against the file's own render thumbnail. Informs the `paint_color` decoder in `@chestnutlabs/gcode-model-renderer` (capability-honest `known`/`approximated`/`unavailable`). |
| [RR-006](RR-006-tube-mesh-memory-and-large-file-budget.md) | Tube-mesh memory cost and the large-file render budget | large-file rendering scalability | **Complete.** Measured the version-agnostic ~2 GB-cgroup OOM on ~1.6 M-segment forced-`tubes` renders: tube geometry is ~23× the memory of lines (~552 B/seg), but `autoDecimation`'s thresholds were calibrated for lines, so large files got no reduction. Fix: a tube-aware **`TUBE_SEGMENT_BUDGET`** (default 400k) that decimates past the budget, **disclosed** via `decimationApplied` and preserving layer boundaries. Verified on the real 47 MB Dragon plate (1.73 M segs): GPU-upload peak 1590→635 MB. Informs `@chestnutlabs/gcode-renderer-three` chunk/scene budgeting. |
