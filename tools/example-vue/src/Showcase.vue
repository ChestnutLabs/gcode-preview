<!--
  ...
  Exercises the real declarative surface of @chestnutlabs/gcode-preview-vue and mirrors the Feature
  Lab's capability-aware UX in Vue idiom: color modes gate on the file's capabilities with a
  plain-language reason, feature-role hiding is a declarative binding, and diagnostics/picking come
  through the component ref's handle (`viewer.preview.controls`). Chrome (light/dark) is separate
...
const mode = computed(() => COLOR_MODE_BY_ID[colorModeId.value]);
const modeReason = computed(() => colorModeReason(colorModeId.value, caps.value));
const colorMode = computed(() => (modeReason.value === '' ? mode.value.build() : undefined));
const legend = computed(() => mode.value.legend ?? null);
...
<template>
  <header class="sc-header">
    ...
    <select class="gp-input" style="width: 220px" :value="fixtureId" @change="load($event.target.value)">
      <optgroup v-for="g in FIXTURE_GROUPS" :key="g.group" :label="g.group">
        <option v-for="i in g.items" :key="i.id" :value="i.id">{{ i.label }}</option>
      </optgroup>
    </select>
    <button class="gp-btn gp-primary" @click="load()">Load</button>
    <div class="sc-spacer" />
  ...
  <div class="sc-main">
    <div class="sc-viewport" @click="onCanvasClick">
      ...
      />
      <div v-if="legend" class="sc-legend-card">
      ...
      <div v-if="segments > 0" class="sc-count">
